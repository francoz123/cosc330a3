 ...
 */
void
Print_results (char title[], int value, int my_rank, int p, MPI_Comm comm)
{
  int *vals = NULL, q;
 ...
 */
int
Global_sum (int my_contrib, int my_rank, int p, MPI_Comm comm)
{
  int sum = my_contrib;
  int temp;
  int partner;
  ...
  while (bitmask > 0)
    {
      ...
      MPI_Sendrecv (&sum, 1, MPI_INT, partner, 0,
		    &temp, 1, MPI_INT, partner, 0, comm, MPI_STATUS_IGNORE);
      sum += temp;
      bitmask >>= 1;
    }
